	...
	switch (data.type) {
		case "char": {
			...
			xmlString = `${header}<ugc more="0">${chars
				.map(
					(v) =>
						`<char id="${v.id}" name="Character" cc_theme_id="${v.theme}" thumbnail_url="https://instance.letsjoincdm.tk/char_thumbs/${v.id}.png" copyable="Y"><tags/></char>`
				)
				.join("")}</ugc>`;
			break;
		}
		...
		case "GET": {
			...
			if (q.movieId && q.type) {
				listAssets(q, makeZip).then((buff) => {
					const type = makeZip ? "application/zip" : "application/xml";
